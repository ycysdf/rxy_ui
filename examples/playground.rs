...
fn setup(mut commands: Commands) {
   ...
   commands.spawn_view_on_root(sample_context());
}
//
...
fn schema_tt(_ctx: SchemaCtx, value: ReadSignal<bool>) -> impl IntoView<BevyRenderer> {
   (
      checkbox(),
      rx(move || value.get().to_string()),
      span("XXX").content("XXXXXXX"),
   )
}
...
fn sample_shared_typed_style_sheet() -> impl IntoView<BevyRenderer> {
   ...
   (
      MenuBtnStyle::def((
         x().width(160).py(8).flex().center().bg_color(basic::GRAY),
         x_hover().bg_color(basic::GRAY),
         x_active().outline_color(basic::GREEN).outline_width(2),
      )),
...
pub fn schema_checkbox(
   ...
   ctx.default_typed_style(CheckboxStyle, || {
      let size = 20;
      let x1 = (
         x().center().size(size).border(1).border_color(basic::GRAY),
         x_hover().bg_color(basic::GRAY),
      );
   });
   div()
...
fn sample_x_res() -> impl IntoView<BevyRenderer> {
   div().gap(10).p(20).flex_col().children((
      ...
      div().p(30).member(x_res(|time: &Time| {
         ...
         ().bg_color(Color::srgb(t.sin(), t.cos(), t.sin() * t.cos()))
      })),
   ))
}
