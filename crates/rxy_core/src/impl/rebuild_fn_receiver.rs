...
use crate::{
   IntoView, MaybeSend, Renderer, RendererNodeId, View, ViewCtx, ViewKey, ViewMember,
   ViewMemberCtx, ViewMemberIndex,
};
   ...
   T: 'static,
{
   ...
   pub fn map<U>(
      self,
      mut map_f: impl FnMut(T) -> U + MaybeSend + 'static,
   ) -> RebuildFnReceiver<R, U> {
      let rebuild_fn_sender_fn = self.1;
      RebuildFnReceiver(
