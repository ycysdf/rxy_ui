...
pub trait RendererViewExt: Renderer {
    fn get_view_state_mut<'w, S: Send + Sync + 'static>(
        ...
        Self::get_node_state_mut::<ViewState<S>>(world, node_id).map(|n| &mut n.0)
    }
    fn get_view_state_ref<'w, S: Send + Sync + 'static>(
        world: &'w Self::World,
        node_id: &Self::NodeId,
    ) -> Option<&'w S> {
        Self::get_node_state_ref::<ViewState<S>>(world, node_id).map(|n| &n.0)
    }
    fn set_view_state<S: Send + Sync + 'static>(
        ...
        Self::set_node_state::<ViewState<S>>(world, node_id, ViewState(state))
    }
}
