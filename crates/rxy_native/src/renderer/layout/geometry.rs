...
impl Val {
   ...
   pub fn resolve(self, parent_size: f32, viewport_size: Vec2) -> Result<f32, ValArithmeticError> {
      match self {
         ...
         Val::Vw(value) => Ok(viewport_size.x as f32 * value / 100.0),
         Val::Vh(value) => Ok(viewport_size.y as f32 * value / 100.0),
         Val::VMin(value) => Ok(viewport_size.x.min(viewport_size.y) as f32 * value / 100.0),
         Val::VMax(value) => Ok(viewport_size.x.max(viewport_size.y) as f32 * value / 100.0),
